...
class ViewModelEmployeeInformationTest {
    ...
    @Test
    fun test_deleteEmployeeApiSuccess() {
        `when`(this.mApiInterface.deleteEmployee("5")).thenAnswer {
            return@thenAnswer Maybe.just(ArgumentMatchers.any<RepositoryViewModel>())
        }

        val observer = mock(Observer::class.java) as Observer<ModelDeleteEmployee>
        this.mViewModelEmployeeInformation.mEmployeeDeleteStatus.observeForever(observer)

        this.mViewModelEmployeeInformation.deleteEmployee("5")

    }
    @Test
    fun test_deleteEmployeeApiError() {
        `when`(this.mApiInterface.deleteEmployee("5")).thenAnswer {
            return@thenAnswer Maybe.error<SocketException>(SocketException("No network here"))
        }
